...
### 这个坑值得单独说：规则加在了没绑定的安全组上

阿里云账号里通常有**好几个**安全组——建实例时自动建的默认组、以前试验留下的空组。
**规则加到哪一个上是有区别的**（这一步纯控制台操作，代码里查不到）。

加错组时的症状很阴：

> **规则列表看起来完全正确，但一点作用都没有。**
> 查规则本身永远查不出问题——规则确实存在，只是它管不到你这台机器。

判据在安全组**列表页**的「**IP 占用数**」那一列：

```
安全组 ID            规则数量   IP 占用数
sg-xxxxxxxxxxxx        6       0（已用）   ← 8001 加在这，白加
sg-yyyyyyyyyyyy        5       0（已用）   ← 也是空的
sg-zzzzzzzzzzzz        3       1（已用）   ← 实例真正绑的是这个，但它没有 8001
```

上例里前两个组规则一条不缺（连 8001 都有），却**一台机器都没绑**。
稳妥做法是别在列表页瞎点：从**实例详情页 →「安全组」标签**进去，
那里列出的才是实际生效的组，直接在那个组上加规则。

**怎么判断自己踩的就是这个坑**——看 SSH：

- `ssh root@<公网IP>` **能连上** → 22 端口通 → 安全组**确实绑上了、也确实生效了**
- 阿里云安全组**只有「允许」规则，没有「拒绝」**，规则之间是叠加的
- 所以「22 通、8001 却超时」**不可能是"被某条规则挡住了"**，
  只能是 8001 那条加在了另一个组上

这个反证很有用：它一步排掉「服务器防火墙没开」「服务没监听」这些方向。
配合着在服务器上确认这三条：

```bash
ss -lntp | grep 8001              # 必须是 0.0.0.0:8001，是 127.0.0.1:8001 就说明只监听了本机
systemctl is-active firewalld     # 通常是 inactive
iptables -S                       # firewalld 没开 ≠ 没有 iptables 规则，那是两件事
```
...
> **如果浏览器报 `502 Bad Gateway`，先怀疑访问者自己开着代理软件。**
> 很多代理（Clash / v2ray / 各种加速器）默认接管**全部**流量，而它们的规则通常只认
> 80/443；撞上 8001 这种非标端口，要么直接拒，要么把请求绕到远端节点、节点连不上就回一个 502。
> 这个 502 **不是服务器发的**——uvicorn 直接对着应用，前面根本没有网关，产生不出 502。
> 给这个公网 IP 加一条直连规则（`IP-CIDR,<公网IP>/32,DIRECT`）再试。
> 关掉代理后如果错误变成**超时**，那才轮到去查安全组（见第 6 步）。
>
> 顺序别搞反：**502 = 有东西回应了，是代理**；**超时 = 没人理你，是网络/安全组**。
